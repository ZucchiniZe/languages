$.getJSON('data.json', function(data) {
  ...
  for(var i=0; i < data.length; i++) {
    ...
    if (luminance < 40) {
      $('#box').append('<div class="square" style="color:#EEE;background:' + data[i][1] + '"><a class="light" href="https://github.com/trending?l=' + data[i][0] + '">' + data[i][0] + '</a></div>')
    } else {
      $('#box').append('<div class="square" style="background:' + data[i][1] + '"><a class="dark" href="https://github.com/trending?l=' + data[i][0] + '">' + data[i][0] + '</a></div>')
    }
  }
});
